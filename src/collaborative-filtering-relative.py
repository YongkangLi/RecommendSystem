...
# subtract the utility matrix from respective average rating
for i in range(mat.shape[0]):
    for j in range(mat.shape[1]):
        mat[i][j] = mat[i][j] - avg[i] if mat[i][j] > 0 else 0

# compute the pearson matrix
sim = pearson(mat, avg)

# compute the prediction ratings
pre = predict(sim, mat, avg, 4)

# recommend k (k=8 here) highest prediction items to each user and compute the sse
evaluate(pre, 8, users_mapping, items_mapping, '../result/collaborative-filtering-relative')
